    ...
    <section className="bg-slate-100/70 px-6 py-20">
      <div className="mx-auto max-w-5xl">
        <div className="mb-8 text-center">
          <span className="text-xs font-bold uppercase tracking-widest text-[#701D10]">Curriculum Structure</span>
        ...
        <div className="mb-7 flex justify-center gap-3">
          {Object.entries(labels).map(([key, label]) => (
            <button key={key} type="button" onClick={() => setPlan(key)} className={`rounded-xl px-6 py-3 font-bold transition ${plan === key ? "bg-[#701D10] text-white shadow-lg" : "bg-white text-slate-600 shadow-sm hover:bg-red-50"}`}>
              {label}
            </button>
          ))}
        </div>
...
function FragmentRows({ group }) {
  return <>
    <tr className="border-b border-red-900 bg-[#701D10] font-bold text-white"><td className="px-6 py-3.5 text-base md:text-lg">{group.category}</td><td className="border-l border-red-800 px-6 py-3.5 text-center text-base md:text-lg">{group.credits}</td></tr>
